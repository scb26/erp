window.Unidex = window.Unidex || {};

(function (ns) {
  // All DOM lookups are grouped here to keep the bootstrap file focused on behavior.
  function collect() {
    var elements = {
      companyForm: document.getElementById("company-form"),
      customerForm: document.getElementById("customer-form"),
      productForm: document.getElementById("product-form"),
      invoiceForm: document.getElementById("invoice-form"),
      companyStateSelect: document.getElementById("company-state"),
      companyNameInput: document.getElementById("company-name"),
      companyTradeNameInput: document.getElementById("company-trade-name"),
      companyGstinInput: document.getElementById("company-gstin"),
      companyPanInput: document.getElementById("company-pan"),
      companyPhoneInput: document.getElementById("company-phone"),
      companyAltPhoneInput: document.getElementById("company-alt-phone"),
      companyEmailInput: document.getElementById("company-email"),
      companyWebsiteInput: document.getElementById("company-website"),
      companyPincodeInput: document.getElementById("company-pincode"),
      companyCityInput: document.getElementById("company-city"),
      companyAddressInput: document.getElementById("company-address"),
      companyAddressLine2Input: document.getElementById("company-address-line2"),
      companyBusinessTypeSelect: document.getElementById("company-business-type"),
      companyBusinessCategorySelect: document.getElementById("company-business-category"),
      
      companyBankHolderInput: document.getElementById("company-bank-holder"),
      companyBankAccountInput: document.getElementById("company-bank-account"),
      companyBankIfscInput: document.getElementById("company-bank-ifsc"),
      companyBankNameInput: document.getElementById("company-bank-name"),
      
      companyUpiInput: document.getElementById("admin-company-upi"),
      companyInvoicePrefixInput: document.getElementById("company-invoice-prefix"),
      companyFinancialYearStartInput: document.getElementById("company-financial-year-start"),
      
      companyLogoInput: document.getElementById("company-logo"),
      companySignatureInput: document.getElementById("company-signature"),
      companyLogoDataInput: document.getElementById("company-logo-data"),
      companySignatureDataInput: document.getElementById("company-signature-data"),
      companyLogoPreview: document.getElementById("company-logo-preview"),
      companySignaturePreview: document.getElementById("company-signature-preview"),
      companyFormMessage: document.getElementById("company-form-message"),

      btnFetchGstin: document.getElementById("btn-fetch-gstin"),
      btnFetchPincode: document.getElementById("btn-fetch-pincode"),
      wizardSteps: Array.prototype.slice.call(document.querySelectorAll(".wizard-step-content")),
      wizardIndicators: Array.prototype.slice.call(document.querySelectorAll(".wizard-step-indicator")),
      btnWizardNext: Array.prototype.slice.call(document.querySelectorAll(".btn-wizard-next")),
      btnWizardBack: Array.prototype.slice.call(document.querySelectorAll(".btn-wizard-back")),
      btnWizardSkip: document.querySelector(".btn-wizard-skip"),
      
      previewLiveLogo: document.getElementById("preview-live-logo"),
      previewLiveBrand: document.getElementById("preview-live-brand"),
      previewLiveAddress: document.getElementById("preview-live-address"),
      previewLiveGstin: document.getElementById("preview-live-gstin"),
      previewLivePhone: document.getElementById("preview-live-phone"),
      previewLiveEmail: document.getElementById("preview-live-email"),
      previewLivePan: document.getElementById("preview-live-pan"),
      customerIdInput: document.getElementById("customer-id"),
      customerNameInput: document.getElementById("customer-name"),
      customerMobileInput: document.getElementById("customer-mobile"),
      customerTypeSelect: document.getElementById("customer-type"),
      customerCompanyNameInput: document.getElementById("customer-company-name"),
      customerStateSelect: document.getElementById("customer-state"),
      customerGstNumberInput: document.getElementById("customer-gst-number"),
      customerEmailInput: document.getElementById("customer-email"),
      customerOpeningBalanceInput: document.getElementById("customer-opening-balance"),
      customerCreditLimitInput: document.getElementById("customer-credit-limit"),
      customerCityInput: document.getElementById("customer-city"),
      customerPincodeInput: document.getElementById("customer-pincode"),
      customerAddressInput: document.getElementById("customer-address"),
      invoiceCustomerNameInput: document.getElementById("invoice-customer-name"),
      invoiceCustomerSelect: document.getElementById("invoice-customer"),
      invoiceCustomerSuggestions: document.getElementById("invoice-customer-suggestions"),
      lineItemsContainer: document.getElementById("line-items"),
      addLineItemButton: document.getElementById("add-line-item"),
      customerList: document.getElementById("customer-list"),
      customerFormTitle: document.getElementById("customer-form-title"),
      customerFormMessage: document.getElementById("customer-form-message"),
      customerSubmitButton: document.getElementById("customer-submit-button"),
      customerCancelButton: document.getElementById("customer-cancel-button"),
      customerRefreshButton: document.getElementById("customer-refresh-button"),
      invoiceHistory: document.getElementById("invoice-history"),
      invoicePreview: document.getElementById("invoice-preview"),
      invoiceFormMessage: document.getElementById("invoice-form-message"),
      printButton: document.getElementById("print-invoice"),
      moduleMenu: document.getElementById("module-menu"),
      moduleMenuTitle: document.getElementById("module-menu-title"),
      activeModuleTitle: document.getElementById("active-module-title"),
      activeModuleDescription: document.getElementById("active-module-description"),
      featureButtons: Array.prototype.slice.call(document.querySelectorAll(".feature-button")),
      moduleScreens: Array.prototype.slice.call(document.querySelectorAll(".module-screen")),
      billingViews: Array.prototype.slice.call(document.querySelectorAll(".billing-view")),
      dashboardTodaySales: document.getElementById("dashboard-today-sales"),
      dashboardTodayExpense: document.getElementById("dashboard-today-expense"),
      dashboardTodayNet: document.getElementById("dashboard-today-net"),
      dashboardChartRangeSelect: document.getElementById("dashboard-chart-range"),
      dashboardSeriesToggle: document.getElementById("dashboard-series-toggle"),
      dashboardSeriesButtons: Array.prototype.slice.call(document.querySelectorAll(".dashboard-series-toggle__button")),
      dashboardChart: document.getElementById("dashboard-chart"),
      adminCompanyName: document.getElementById("admin-company-name"),
      adminCompanyUpi: document.getElementById("admin-company-upi"),
      adminCompanyGstin: document.getElementById("admin-company-gstin"),
      adminCompanyState: document.getElementById("admin-company-state"),
      featureRail: document.getElementById("feature-rail"),
      sidebarToggle: document.getElementById("sidebar-toggle"),
      subtotalValue: document.getElementById("subtotal-value"),
      cgstValue: document.getElementById("cgst-value"),
      sgstValue: document.getElementById("sgst-value"),
      igstValue: document.getElementById("igst-value"),
      grandTotalValue: document.getElementById("grand-total-value"),
      inventorySearchInput: document.getElementById("inventory-search"),
      btnAddNewProduct: document.getElementById("btn-add-new-product"),
      btnBulkImport: document.getElementById("btn-bulk-import"),
      lowStockAlertSection: document.getElementById("low-stock-alert-section"),
      lowStockCount: document.getElementById("low-stock-count"),
      lowStockList: document.getElementById("low-stock-list"),
      productInventoryList: document.getElementById("product-inventory-list"),
      fabAddProduct: document.getElementById("fab-add-product"),
      productModal: document.getElementById("product-modal"),
      productModalTitle: document.getElementById("product-modal-title"),
      closeProductModal: document.getElementById("close-product-modal"),
      productSubmitBtn: document.getElementById("product-submit-btn"),
      productCancelBtn: document.getElementById("product-cancel-btn"),
      productIdInput: document.getElementById("product-id"),
      productNameInput: document.getElementById("product-name"),
      productBarcodeInput: document.getElementById("product-barcode"),
      productHsnInput: document.getElementById("product-hsn"),
      productPriceInput: document.getElementById("product-price"),
      productGstInput: document.getElementById("product-gst"),
      productStockInput: document.getElementById("product-stock"),
      // Vendor Elements
      vendorList: document.querySelector(".vendor-list-container"),
      btnAddVendor: document.getElementById("btn-add-vendor"),
      btnQuickAddVendor: document.getElementById("btn-quick-add-vendor"),
      vendorModal: document.getElementById("vendor-modal"),
      vendorModalTitle: document.getElementById("vendor-modal-title"),
      closeVendorModal: document.getElementById("close-vendor-modal"),
      vendorForm: document.getElementById("vendor-form"),
      vendorIdInput: document.getElementById("vendor-id"),
      vendorNameInput: document.getElementById("vendor-name"),
      vendorPhoneInput: document.getElementById("vendor-phone"),
      vendorGstinInput: document.getElementById("vendor-gstin"),
      vendorStateSelect: document.getElementById("vendor-state"),
      vendorOpeningBalanceInput: document.getElementById("vendor-opening-balance"),
      vendorSubmitBtn: document.getElementById("vendor-submit-btn"),
      vendorCancelBtn: document.getElementById("vendor-cancel-btn"),
      vendorSearchInput: document.querySelector("#product-vendors input[type='search']"),
      
      // Purchase Elements
      purchaseHistorySearch: document.getElementById("purchase-history-search"),
      btnPurchaseHistoryFilter: document.getElementById("btn-purchase-history-filter"),
      purchaseHistoryDateFilter: document.getElementById("purchase-history-date-filter"),
      purchaseHistoryList: document.querySelector(".purchase-history-list"),
      purchaseForm: document.getElementById("purchase-bill-form"),
      purchaseVendorName: document.getElementById("purchase-vendor-name"),
      purchaseVendorId: document.getElementById("purchase-vendor"),
      purchaseVendorSuggestions: document.getElementById("purchase-vendor-suggestions"),
      purchaseDate: document.getElementById("purchase-date"),
      purchaseNumber: document.getElementById("purchase-number"),
      purchaseLineItems: document.getElementById("purchase-line-items"),
      addPurchaseLineBtn: document.getElementById("add-purchase-line-item"),
      purchaseSubtotal: document.getElementById("purchase-subtotal-value"),
      purchaseGst: document.getElementById("purchase-gst-value"),
      purchaseGrandTotal: document.getElementById("purchase-grand-total-value"),
      purchaseAmountPaid: document.getElementById("purchase-amount-paid"),
      purchasePaymentMethod: document.getElementById("purchase-payment-method"),
      purchaseBalanceDue: document.getElementById("purchase-balance-due-value"),
      purchaseNotes: document.getElementById("purchase-notes"),
      purchaseFormMessage: document.getElementById("purchase-form-message"),
      cancelPurchaseBtn: document.getElementById("cancel-purchase-btn")
    };

    elements.isReady = !!(
      elements.companyForm &&
      elements.customerForm &&
      elements.productForm &&
      elements.invoiceForm
    );

    return elements;
  }

  ns.dom = {
    collect: collect
  };
})(window.Unidex);



